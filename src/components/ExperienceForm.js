...
export default function ExperienceForm({ user, business, onBack, onSaved }) {
  ...
  async function submit() {
    ...
    try {
      ...
      await supabase.from('experience_responses')
    ...
    <div style={s.page}>
      <div style={s.card}>
        ...
        <div style={s.bizInfo}>
          ...
          <span style={s.bizMeta}>{business.category} • {business.city}, {business.state}</span>
        </div>
          ...
          <>
            ...
            <div style={s.options}>
              {current.options.map(opt => {
                ...
                const extra = sel ? (opt.signal === 'positive' ? s.selGood : opt.signal === 'neutral' ? s.selMid : s.selBad) : {}
                return (
                  <button key={opt.label} style={{ ...s.optBtn, ...extra }} onClick={() => selectAnswer(current.key, opt)}>
                    {opt.label}
                  </button>
                )
              })}
...
const s = {
  ...
  bizMeta: { fontSize: 12, color: '#6B6B6B', marginTop: 3 },
  ...
  optBtn: { width: '100%', padding: '16px 20px', background: '#FAFAF8', border: '1.5px solid #E2E0DA', borderRadius: 12, fontSize: 15, cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit', color: '#0E0E0E' },
