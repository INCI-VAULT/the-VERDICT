...
export default function Dashboard({ user, onAddBusiness, onViewBusiness, onSignOut }) {
  ...
  async function fetchBusinesses() {
    setLoading(true)
    // Fetch ALL businesses — shared across all users
  ...
  function getAggregateOutcome(biz) {
    if (!biz.experiences?.length) return null
    // Use the most recent experience outcome as the current verdict
    return getLatest(biz)
  }
