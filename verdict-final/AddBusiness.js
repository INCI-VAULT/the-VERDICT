...
export default function BusinessDetail({ user, business, onBack, onEvaluate }) {
  ...
  // Overall outcome based on ALL experiences
  function getOverallOutcome() {
    if (!experiences.length) return null
    const criticalCount = experiences.filter(e => e.outcome === 'High Risk' || e.outcome === 'Risk').length
    const cautionCount  = experiences.filter(e => e.outcome === 'Caution').length
    const total         = experiences.length
    if (criticalCount / total >= 0.5) return experiences[0].outcome // most recent if majority bad
    if (cautionCount  / total >= 0.5) return 'Caution'
    return 'Clear'
  }

  const overallOutcome = getOverallOutcome()
  const oc = overallOutcome ? OUTCOME_CONFIG[overallOutcome] : null
      ...
      <div style={s.inner}>
        ...
        <div style={s.header}>
          ...
          <div style={s.headerRight}>
            {oc && (
              <div style={{ ...s.overallBadge, background: oc.bg, color: oc.text, border: `1px solid ${oc.dot}` }}>
...
const s = {
  ...
  overallBadge: { display: 'flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 20, fontSize: 13, fontWeight: 500 },
