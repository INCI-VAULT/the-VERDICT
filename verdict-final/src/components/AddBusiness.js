...
import { supabase, HOME_CATEGORIES, PERSONAL_CATEGORIES } from '../lib/supabase'
...
export default function AddBusiness({ user, onBack, onSaved }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    if (!name.trim() || !serviceType || !category) { setError('All fields are required.'); return }
    setError(''); setLoading(true)
    try {
      const { error } = await supabase.from('businesses').insert({
        user_id: user.id, name: name.trim(),
        service_type: serviceType, category, notes: notes.trim() || null
      })
  ...
  return (
    <div style={s.page}>
      <div style={s.card}>
        ...
        <p style={s.sub}>Add a service provider you want to track.</p>
        ...
        <form onSubmit={handleSubmit} style={s.form}>
          ...
          <div style={s.field}>
            ...
            <textarea style={{ ...s.input, height: 80, padding: '12px 16px', resize: 'vertical' }} placeholder="Any notes about this business…" value={notes} onChange={e => setNotes(e.target.value)} />
          </div>
