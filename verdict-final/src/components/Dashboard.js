...
export default function Dashboard({ user, onAddBusiness, onViewBusiness, onSignOut }) {
  ...
  return (
    <div style={s.wrap}>
      ...
      <main style={s.main}>
        <div style={s.topBar}>
          <h2 style={s.pageTitle}>Your Businesses</h2>
        ...
        {loading ? (
          ...
          <div style={s.emptyState}>
            ...
            <p style={s.emptyText}>Add a business to start tracking experiences.</p>
            <button style={s.addBtn} onClick={onAddBusiness}>Add Your First Business</button>
          </div>
        ) : (
          <div style={s.grid}>
            {businesses.map(biz => {
                ...
                <div key={biz.id} style={s.card} onClick={() => onViewBusiness(biz)}>
                  <div style={s.cardTop}>
                    <div style={s.cardLeft}>
                      ...
                      <div style={s.bizCat}>{biz.category} • {biz.service_type === 'home' ? '🏠 Home' : '💇 Personal'}</div>
                    </div>
                    {latest && oc ? (
...
const s = {
  ...
  cardTop:    { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, gap: 12 },
  cardLeft:   { flex: 1, minWidth: 0 },
  bizName:    { fontSize: 16, fontWeight: 500, color: '#0E0E0E', marginBottom: 3 },
  bizCat:     { fontSize: 12, color: '#6B6B6B', textTransform: 'capitalize' },
